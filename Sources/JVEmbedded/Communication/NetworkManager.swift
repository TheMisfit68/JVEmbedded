...
// MARK: - Singleton
public final class NetworkManager: Singleton {
	...
	private init() throws(NetworkError) {
		...
		let wifiHandler: esp_event_handler_t? = NetworkManager.wifiCallback
		let ipHandler: esp_event_handler_t? = NetworkManager.ipCallback
...
// C-bridging callbacks as static properties
extension NetworkManager{
	...
	private static let wifiCallback: esp_event_handler_t = { context, eventBase, eventId, eventData in
		guard let ev = WiFiEventID(rawValue: eventId), let manager = NetworkManager.shared else { return }
		switch ev {
			case .staStart:
#if DEBUG
	...
	private static let ipCallback: esp_event_handler_t = { context, eventBase, eventId, eventData in
		guard let ev = IPEventID(rawValue: eventId), let manager = NetworkManager.shared else { return }
		switch ev {
			case .gotIP, .gotIP6:
				manager.retryCount = 0
#if DEBUG
